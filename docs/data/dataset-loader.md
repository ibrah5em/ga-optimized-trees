...
1. Empty or too small datasets (\<10 samples)
1. Minimum class size validation (warns if any class \< 5 samples)
1. Severe class imbalance detection (>10:1 ratio)
1. Zero variance feature detection
1. Feature-target dimension consistency
1. Data type validation (converts non-numeric where possible)
...
1. Last column: target variable (must be numeric or categorical)
1. All other columns: features (numeric or categorical)
1. Missing values: represented as empty strings or NA
1. Categorical features: automatically one-hot encoded
1. Categorical targets: automatically label encoded to integers
