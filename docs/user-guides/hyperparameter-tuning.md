...
1. **Bayesian Sampling**: TPE suggests promising configurations
1. **Cross-Validation**: Evaluate on CV folds
1. **Early Stopping**: Prune unpromising trials
1. **Select Best**: Return configuration with highest score
1. **Export Config**: Save to YAML for reproducibility
...
def custom_objective(trial):
    ...
    params = {
        ...
        "node_complexity_weight": trial.suggest_float(
            "node_complexity_weight", 0.3, 0.8
        ),
        "feature_coherence_weight": trial.suggest_float(
            "feature_coherence_weight", 0.05, 0.35
        ),
    }
...
study = optuna.load_study(
    study_name="my_optimization", storage="sqlite:///optuna_studies.db"
)
...
1. Compare against CART baseline
1. Show statistical tests
...
1. Reduce `cv_folds` (5 → 3)
1. Use smaller search space
1. Enable pruning aggressively
...
1. Check for bugs in objective function
1. Use different sampler
...
1. Increase `n_startup_trials`
1. Check objective function returns valid values

```python
# Disable pruning for debugging
study = optuna.create_study(
    pruner=optuna.pruners.NopPruner(), direction="maximize"  # No pruning
)
```
...
1. Reduce CV folds
1. Clear population after each trial
