...
def test_sample_threshold_and_leaf_sampling():
    ...
    node = create_bayesian_internal_node(feature_idx=0, threshold_mean=0.0, threshold_std=1.0, threshold_dist_type="normal")
...
def test_get_soft_decision_prob_normal():
    ...
    node = create_bayesian_internal_node(feature_idx=0, threshold_mean=0.5, threshold_std=0.1, threshold_dist_type="normal", left_child=left, right_child=right)
    x_row = np.array([0.6])
