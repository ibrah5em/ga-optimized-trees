...
def run_multiple_configs(X_train, y_train, X_test, y_test, base_config, n_configs=10):
    ...
    for acc_weight in np.linspace(min_weight, max_weight, n_configs):
        weight_configs.append({"accuracy": acc_weight, "interpretability": 1.0 - acc_weight})
    ...
    for i, weights in enumerate(weight_configs, 1):
        ...
        feature_ranges = {j: (X_train[:, j].min(), X_train[:, j].max()) for j in range(n_features)}
...
def plot_pareto_front(
    results, dataset_name, base_config, save_path="results/figures/pareto_front.png"
):
    ...
    ax1.scatter(
        base_result["interpretability"],
        base_result["test_accuracy"],
        s=400,
        color="red",
        marker="*",
        edgecolors="black",
    ...
    ax1.set_title(
        f"Pareto Front: Accuracy vs Interpretability\n{dataset_name} Dataset",
        fontsize=15,
        fontweight="bold",
    )
    ...
    ax2.set_title(
        f"Trade-off: Accuracy vs Tree Complexity\n{dataset_name} Dataset",
        fontsize=15,
        fontweight="bold",
    )
    ...
    if nodes:
        ...
        ax2.axvspan(
            min(nodes), low_complexity, alpha=0.1, color="blue", label="Low Complexity Zone"
        )
...
def main():
    ...
    parser.add_argument(
        "--n-configs", type=int, default=10, help="Number of weight configurations to try"
    )
    ...
    results = run_multiple_configs(
        X_train, y_train, X_test, y_test, config, n_configs=args.n_configs
    )
    ...
    print(
        f"  - Your current choice → {config['fitness']['weights']['accuracy']:.0%} accuracy weight"
    )
