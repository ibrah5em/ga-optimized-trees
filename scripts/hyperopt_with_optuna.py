...
class OptimizationTracker:
    ...
    def __init__(self, study_name: str, output_dir: str = "results/optimization"):
        ...
        self.history = {"trials": [], "best_values": [], "best_params": [], "timestamps": []}
...
def create_study(
    study_name: str, storage: Optional[str] = None, resume: bool = False
) -> optuna.Study:
    """
...
def save_best_config(study: optuna.Study, output_path: str = "configs/optimized.yaml"):
...
Examples:
  ...
  python scripts/hyperopt_with_optuna.py --study-name my_study --resume
    ...
    parser.add_argument(
        "--storage", type=str, help="Optuna storage URL (e.g., sqlite:///optuna.db)"
    )
    parser.add_argument("--resume", action="store_true", help="Resume previous study")

    # Output
    parser.add_argument(
        "--output", type=str, default="configs/optimized.yaml", help="Output config file path"
    )
